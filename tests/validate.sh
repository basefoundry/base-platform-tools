...
main() {
    ...
    require_file bin/caff || failed=1
    require_file bin/sort-in-place || failed=1
    ...
    require_file cli/bash/commands/caff/README.md || failed=1
    require_file cli/bash/commands/caff/caff.sh || failed=1
    require_file cli/bash/commands/caff/tests/caff.bats || failed=1
    require_file cli/bash/commands/sort-in-place/README.md || failed=1
    ...
    require_file tests/bats_helper.bash || failed=1
    require_file tests/validate.sh || failed=1
    require_executable tests/validate.sh || failed=1
    require_executable bin/caff || failed=1
    require_executable bin/sort-in-place || failed=1
    ...
    require_text base_manifest.yaml '^  caff: bin/caff$' || failed=1
    require_text base_manifest.yaml '^  cli-check: tests/validate\.sh$' || failed=1
    require_text base_manifest.yaml '^  sort-in-place: bin/sort-in-place$' || failed=1
    require_text README.md 'Base owns the workstation control plane' || failed=1
    require_text README.md 'CLI Layout' || failed=1
    require_text README.md 'basectl run base-platform-tools caff' || failed=1
    require_text bin/README.md 'base-wrapper' || failed=1
    require_text bin/caff 'BASE_HOME' || failed=1
